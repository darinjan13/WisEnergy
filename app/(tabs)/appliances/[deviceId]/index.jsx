...
export default function DeviceDetails() {
  ...
  useEffect(() => {
    if (
      ...
      selectedAppliance !== device.appliance_name
    ) {
  ...
  const handleDeleteConfirmed = async () => {
    const applianceRef = ref(
      db,
      `appliances/${userId}/${deviceId}/${applianceId}`
    );
  ...
  const onAdd = async ({ appliance_name, appliance_nickname }) => {
    ...
    let appliancesRef = ref(
      db,
      `appliances/${userId}/${deviceId}/${newApplianceName}`
    );
    ...
    <View className="p-5" style={{ paddingTop: insets.top + 18 }}>
      ...
      <View className="mb-4">
        ...
        >
          ...
          <Text className="text-md font-bold text-gray-400 ml-2">
            Back to Devices
          </Text>
        </TouchableOpacity>
      </View>
      <Text className="text-2xl font-bold text-green-700">
        Device Name: {device.device_nickname}
      </Text>
      ...
      <Tooltip
        ...
        content={`Devices track energy for one appliance at a time. If you switch the appliance (e.g., from Refrigerator to Electric Fan), the device will start recording consumption for the new appliance. You can switch back anytime.`}
        from="Devices"
      />
