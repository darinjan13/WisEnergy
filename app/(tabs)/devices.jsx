  ...
  const handleAddDevice = async () => {
    ...
    try {
      ...
      const existingDevice = devices.find(
        (device) =>
          device.id === selectedUnpairedDevice && device.status === "paired"
      );
      ...
      const addDeviceStatus = await addDevice(
        deviceCode,
        selectedUnpairedDevice,
        device_nickname
      );
  ...
  return (
    <View>
          ...
          <AutoSkeletonIgnoreView>
            <View className="flex-row items-center justify-between mb-10">
              ...
              <TouchableOpacity
                ...
                className="rounded-3xl bg-white items-center justify-center"
              >
                <Feather
                  className="p-1"
                  name="plus"
      ...
      <Tooltip
        ...
        content={`Lists all your connected WisEnergy devices.\n Pair a new one to start real-time energy monitoring.`}
        from="Devices"
      />
      <Modal
        animationType="fade"
      ...
      >
        ...
        >
          <View className="bg-white rounded-xl p-6 w-11/12">
            <Text
              testID="modal-title"
              className="text-xl font-bold mb-4 text-[#2E4F4F]"
            >
              {action === "edit" ? "Edit Device" : "Add Device"}
            </Text>
            <TextInput
            ...
            {action !== "edit" ? (
              <>
                ...
                <View className="border border-gray-300 rounded-xl mb-4 overflow-hidden">
                  <Picker
                    ...
                    onValueChange={(itemValue) =>
                      setSelectedUnpairedDevice(itemValue)
                    }
                  >
                    <Picker.Item
                      label="Select Device"
                    ...
                    {unpairedDevices.map((device, idx) => (
                      <Picker.Item
                        key={idx}
                        label={
                          device.label || device.name || `Device ${idx + 1}`
                        }
                ...
                <View className="mb-4">
                  <TextInput
                    className="border border-gray-300 rounded-lg p-4 bg-gray-50 text-black"
                    placeholder="Enter Device ID"
                    placeholderTextColor="#9CA3AF"
                    value={selectedUnpairedDevice}
                    onChangeText={(text) => setSelectedUnpairedDevice(text)}
                  />
                </View>
                ...
                <View className="border border-gray-300 rounded-lg mb-6 bg-gray-50 flex-row items-center">
                  ...
                  <IconButton
                    ...
                    icon={showPassword ? "eye-off" : "eye-outline"}
                    iconColor="gray"
                  />
                </View>
              </>
